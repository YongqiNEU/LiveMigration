...
void savingCheckPointImage();
void parseSectionHeader(char * header, struct memorySection * section);
int readLine(int checkpoint_image_fd, char line[256]);
void writeToImage(int fd, struct memorySection * section);
...
void parseSectionHeader(char * header, struct memorySection * section) {
    sscanf(header, "%p-%p %c%c%c%c %x",
           &section->start, &section->end,
           &section->permissions[0], &section->permissions[1], 
           &section->permissions[2], &section->permissions[3], &section->offset);
}